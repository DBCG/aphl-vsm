  ...
  return cdrResponse?.entry
    ...
    ?.map((cdrBundle: BundleEntry) => {
      ...
      if (valueSets.length > 1) {
        ...
        getLogger().error(`No ValueSets found in bundle for ${bundle.link?.[0]?.url}`)
        return
      }
    })
...
const gatherVsToUpdate = (toUpdateCollection: CDRResponseCollection) => {
  ...
  for (const url in toUpdateCollection) {
    ...
    if (!authorativeValueSet) {
      getLogger().error(`No authoritative ValueSet found for ${cdrValueSet.id}`)
      continue
    }
    ...
    if (needsUpdate) {
      getLogger().info(`ValueSet ${cdrValueSet.id} needs to be updated`)
    ...
    await Promise.all(
      cachedCdrVS.map(async (valueset) => {
        if (isVsmAuthored(valueset)) {
          getLogger().info(`Skipping VSM authored ValueSet ${valueset.id}`)
          return
        }
        ...
        if (!vsComparatorResponses || vsComparatorResponses?.total == 0) {
          getLogger().error(`No ValueSets found in bundle for ${valueset.url} from authoritative source ${value}`)
...
valueSetUpdateQueue.process(async function (job, done) {
  ...
  if (clonedUrls?.length === 0 || programId == null) {
    getLogger().error('Urls and ProgramID required for valueset update worker')
    done()
  }
  const maxIterations = Math.ceil(clonedUrls.length / MAX_JOB_SIZE) // Scale batch number
  let iteration = 0
  getLogger().info(`Starting job id: ${job.id} with urls ${clonedUrls.length} and dividing into ${maxIterations} batches`)
  ...
  while (clonedUrls.length > 0) {
    ...
    if (progress >= 100) {
      getLogger().info('Progress: 99, begin checking for update to finish')
      job.progress(99) // prevent job from finishing
      break
    } else {
      getLogger().info('Progress: ' + (iteration / maxIterations) * 100)
  ...
  if (batchedJobs?.length > 0) {
    ...
    while (!didFinishUpdate) {
      ...
      getLogger().info('New VS ids', allBatchJobIds)
      getLogger().info('current VS ids', currentVsIds)
      if (anyIntersection?.length) {
        getLogger().info('Update finished')
        didFinishUpdate = true
        break
      } else {
        getLogger().info('Waiting for update to finish, leaf values intersection ids did not match')
        await sleep(5000)
      }
    }
  }
