...
export default async function handler(
  ...
  if (req.method === 'GET') {
    if (session?.idToken != null) {
      ...
      return res.redirect(`${process.env.KEYCLOAK_ISSUER}/protocol/openid-connect/logout?id_token_hint=${session.idToken}&post_logout_redirect_uri=${process.env.NEXTAUTH_URL}`)
    } else {
