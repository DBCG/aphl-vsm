  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      if (!is.library(program)) {
        return res.status(400).json({ error: `Program must be in Library format.` })
      }
      ...
      if (!grouperLibraryCanonical) {
        return res.status(404).json({ error: `Grouper library must include url.` })
      }

      const grouperSearchResult = await fetchGrouperLibrary(fhirCdrClient, grouperLibraryCanonical, grouperStatus)
