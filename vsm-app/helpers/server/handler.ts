...
type action<T extends NextApiRequest> = (req: T, res: NextApiResponse, logger: Logger, session: VSMSession) => Promise<any>
...
const handler = <U extends NextApiRequest, T extends handlerObjs<U>>(methodHandlers: T) => async <T extends NextApiRequest>(req: T, res: NextApiResponse): Promise<void> => {
  const childLogger =  logger.child({ 'logId': uuidv4() })
  ...
  if (!req.method || !(requestTypes.includes(req.method as any)) || !methodFn) {
    childLogger.error(`${req.method} not allowed for ${req.url}`)
    return res.status(405).json({ error: 'Method not allowed' })
  }

  childLogger.info(`Request: ${req?.method} ${req?.url}`)
  ...
  try {
    ...
    if (!isAuthorized) {
      childLogger.error(`Role: ${role} is not authorized for ${req?.url}`)
      return res.status(401).json({ error: 'Unauthorized' })
    }
    return await action(req, res, childLogger, session)
  } catch (error: any) {
