...
if (process.env.KEY == null) {
  getLogger().error('Symmetric key not found in env variables')
...
class APITokenHandler {
  ...
  private async getUserIV(user: string | KeyCloakUser) {
    if (typeof user !== 'string') {
      ...
      if (user?.attributes?.iv?.[0] == null) {
        ...
        getLogger().info('IV Not found, Generating IV for user: ' + userId)
  ...
  private async retrieveStoredAttributes(userId: string | KeyCloakUser) {
    getLogger().info('Retrieving stored attributes for userId: ' + userId)
    ...
    try {
      ...
      getLogger().error(`Error retrieving attributes for userId: ${userId} from Keycloak: ${error}`)
      this.resetState()
      throw error
    }
  }
  ...
  private async deleteCredsKeyCloak(userId: string, serverId: string) {
    getLogger().info(`Deleting creds for serverId: ${serverId} for user: ${userId}`)
    const url = `${KEYCLOAK_BASE_URL}/admin/realms/aphl/users/${userId}`
    const headers = new Headers()
    headers.set('Content-Type', 'application/json')
    headers.set('Authorization', `Bearer ${this._cacheJWT}`)
    ...
    try {
      ...
      if (response.status === 204) {
        getLogger().info(`Successfully deleted creds for serverId: ${serverId}`)
        return response
      } else {
        getLogger().error(`Failed to delete creds for serverId: ${serverId} in Keycloak: ${response.statusText}`)
        throw new Error(`Failed to delete creds for serverId: ${serverId} in Keycloak: ${response.statusText}`)
      }
    } catch (error) {
      getLogger().error(`Error deleting creds in Keycloak: ${error}`)
      this.resetState()
      throw error
    }
  }
  ...
  private async storeCredsKeyCloak(user: string | KeyCloakUser, key: string, value: string) {
    ...
    getLogger().info(`Setting creds '${key}' in Keycloak for user: ${userId}`)
    const url = `${KEYCLOAK_BASE_URL}/admin/realms/aphl/users/${userId}`
    const headers = new Headers()
    headers.set('Content-Type', 'application/json')
    headers.set('Authorization', `Bearer ${this._cacheJWT}`)
    ...
    try {
      ...
      if (response.status === 204) {
        getLogger().info(`Successfully store key: '${key}' in keycloak`)
        return response
      } else {
        getLogger().error(`Failed to store ${key} in Keycloak: ${response.statusText}`)
        throw new Error(`Failed to store ${key} in Keycloak: ${response.statusText}`)
      }
      ;``
    } catch (error) {
      getLogger().error(`Error setting API key in Keycloak: ${error}`)
      this.resetState()
      throw error
    }
  }
  ...
  private async renewKeyCloakToken() {
    if (this._cacheJWT == null || Date.now() > (this._cacheJWTExpiry || -1)) {
      getLogger().info('Renewing ServerSide JWT Token')
      ...
      try {
        ...
        getLogger().error(`Error generating token: ${error}`)
        throw error
      }
    }
  }
