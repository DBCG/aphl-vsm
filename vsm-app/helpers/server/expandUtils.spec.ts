...
describe('buildSearchUrl', () => {
  it('returns the proper base if no version present', () => {
    ...
    expect(buildSearchUrl({ leafUrl, leafVersion })).toBe('/ValueSet?url=http://test.com&_sort=version')
  })

  it('returns url with version if specified', () => {
    const leafUrl = 'http://test.com'
    const leafVersion = '2.0.0'
    expect(buildSearchUrl({ leafUrl, leafVersion })).toBe('/ValueSet?url=http://test.com&_sort=version&version=2.0.0')
  })
})