...
For example, it may be useful for VSM authors to "tag" ValueSets with extra information to help them categorize, connect, and filter concepts while building a Program in the context of the VSM.
...
- An extension to designate the "priority" of ValueSets (Emergent, Priority, or Routine). This is done manually by users.

### Why Authoritative Source?
Knowing the Authoritative Source allows us to update ValueSets when necessary and allow users to "pin" specific versions of ValueSets to a Program. If we did not know which server they came from, neither action would be possible.
...
Priority is currently added at the Program Library level, and trickles down through a batch update to all of the associated Leaf ValueSets in the form of a usage context item.
